...
Given a sample of data $X = \{x_1,x_2,..., x_n\}$ with n observations from a d-variate distribution an isolation tree would take an attribute $q$of an observation and compare it to a splitting value p. The test is $q < p$: if $q < p$, then the point would be assigned to $T_l$, else it would be assigned to $T_r$.
To build an isolation tree, we recursively divide X by randomly selecting an attribute $q \in\{1,...,d\}$ and splitting value $p$, which is some number in between the maximum and minimum of the $q$th attribute of all data points.
The test splits the data points into $T_l$ and $T_r$ and the process continues recursively until either:
...
$s(x,n) = 2^{-E(h(x)/c(n))}$, where  $E(h(x))$ is the expected $h(x)$ from a collection of isolation trees, $c(n)  = 2H(n − 1) − (2(n − 1)/n)$; average path length of unsuccessful search in Binary Search Tree, $H(i)$ is the harmonic number and it can be estimated by $ln(i) + 0.5772156649$ (Euler’s constant).More about why $c(n)$ is used in references.
